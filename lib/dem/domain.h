...
public:
    ...
    void WriteXDMF_User (Array<double> strainEF, char const * FileKey);
...
inline void Domain::Solve (double tf, double dt, double dtOut, ptFun_t ptSetup, ptFun_t ptReport, char const * TheFileKey, size_t VOut, size_t TheNproc, double minEkin)
{
    ...
    //std::cout << ListPosPairs.Size() << std::endl;
...
inline void Domain::WriteXDMF_User ( Array<double> strainEF, char const * FileKey)
{
    size_t N_Faces = 0;
    size_t N_Verts = 0;
    for (size_t i=0; i<Particles.Size(); i++) 
    ...
    if (Xmax-Xmin>Alpha)
    {
        ...
        for (size_t i=0;i<Nproc;i++)
        {
            for (size_t j=0;j<MTD[i].LPC.Size();j++)
            {
                ...
                if (FricCoeff.count(p)==1)
                {
                    CPxInteractons[CPxInteractons.Size()-1]->Mu = FricCoeff[p];                    
                }
            }
        }
    ...
    if (Ymax-Ymin>Alpha)
    {
        ...
        for (size_t i=0;i<Nproc;i++)
        {
            for (size_t j=0;j<MTD[i].LPC.Size();j++)
            {
                ...
                if (FricCoeff.count(p)==1)
                {
                    CPyInteractons[CPyInteractons.Size()-1]->Mu = FricCoeff[p];                    
                }
            }
        }
    ...
    if ((Xmax-Xmin>Alpha)&&(Ymax-Ymin>Alpha))
    {
        ...
        for (size_t i=0;i<Nproc;i++)
        {
            for (size_t j=0;j<MTD[i].LPC.Size();j++)
            {
                ...
                if (FricCoeff.count(p)==1)
                {
                    CPxyInteractons[CPxyInteractons.Size()-1]->Mu = FricCoeff[p];                    
                }
            }
        }
