...
int main(int argc, char **argv) try
{
    ...
    size_t ndiv = 75; //number of divisions per z lenght 
    ...
    double Dz   = 3.0*(xmax(2)-xmin(2))/ndiv;
    double Bc   = Dz+xmin(2);
    double Lz   = (xmax(2)-xmin(2))*2/3;
    dat.Bc      = Bc;
    dat.Lz      = Lz;
    dat.xmin    = xmin(0);
    dat.xmax    = xmax(0);
    ...
    dat.ymax    = xmax(1);
    dat.zmin    = xmin(2);
    dom.ResizeDomain(xmin-1.0*(xmax(2)-xmin(2)),xmax+1.0*(xmax(2)-xmin(2)), Dz); //Mesh size
    ...
    dat.Tf = Tf;  
    dat.Vb = 0.5;  //velocity applied at particles   
